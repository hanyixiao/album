...
void lcd_rgb_draw(uint16_t startx, uint16_t starty,uint16_t width,uint16_t height,uint8_t *buf)
{  
    uint16_t t[2];
    uint16_t *ptr = (uint16_t *)buf;
    uint16_t *p = ptr;
    lcd_set_area(startx, starty, startx + width - 1, starty +height - 1);
    ...
    tft_write_word(p, (width *height / 2), 0);
