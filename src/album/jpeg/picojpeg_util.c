   ...
   *err = 0;
   ...
   for (;;)
   {
      ...
      else
      {
         ...
         // printf("[info]:image_info.m_MCUHeight & widht %d %d ",image_info.m_MCUHeight,image_info.m_MCUWidth);
                  ...
                  for (by = 0; by < by_limit; by++)
                  {
                     ...
                     for (bx = 0; bx < bx_limit; bx++)
                     {
                        if (rgb565)
                        {
                           ...
                           pDst[0] = color >> 8 & 0xff;
                           pDst[1] = color & 0xff;
                           // printf("[INFO]:use rgb888ToRGB565 function\n");
                           // printf("size rgb %ld %ld\n",pSrcR-pSrcB,pSrcG-pSrcB);
                        }
                        else
                        {
                           pDst[0] = *pSrcR;
